...
module.exports = async (req, res) => {
  ...
  try {
        ...
        return new Promise((resolve) => {
          upload.single('image')(req, res, async (err) => {
            ...
            try {
              ...
              const savedUser = await User.findById(auth.user.id).lean();
              console.log('💾 After save - ownedHives in DB:', JSON.stringify(savedUser.ownedHives, null, 2));
    ...
    if (basePath.match(/^\/[^/]+\/hives$/) && method === 'POST') {
      ...
      if (!hasHive) {
        // Look for existing hive data from any user
        const allUsers = await User.find({}, 'ownedHives');
        let existingHiveData = null;
        
        for (const u of allUsers) {
          const foundHive = u.ownedHives?.find(h => {
            const hId = typeof h === 'string' ? h : h?.id;
            return hId === hiveId;
          });
          if (foundHive && typeof foundHive === 'object') {
            existingHiveData = foundHive;
            break;
          }
        }
        
        // Transfer existing hive data or create with defaults
        const hiveToAdd = existingHiveData ? {
          id: hiveId,
          name: existingHiveData.name || `Úľ ${hiveId.replace('HIVE-', '')}`,
          location: existingHiveData.location || '',
          color: existingHiveData.color || '#fbbf24',
          image: existingHiveData.image || undefined,
          device: existingHiveData.device || undefined
        } : {
          id: hiveId,
        ...
        Object.keys(hiveToAdd).forEach(key => {
          if (hiveToAdd[key] === undefined) delete hiveToAdd[key];
        });
        
        user.ownedHives.push(hiveToAdd);
        user.markModified('ownedHives'); // Required for Mixed type arrays
        await user.save();
      }
          ...
          upload.single('image')(req, res, async (err) => {
            ...
            try {
              ...
              if (req.file) {
                ...
                console.log('📸 Image uploaded to:', user.ownedHives[hiveIndex].image);
              }

              console.log('💾 Saving hive:', JSON.stringify(user.ownedHives[hiveIndex], null, 2));
              user.markModified('ownedHives'); // Required for Mixed type arrays
              await user.save();
              
              // Verify what was saved
              const savedUser = await User.findById(auth.user.id).lean();
              console.log('✅ After PATCH save - hive in DB:', JSON.stringify(savedUser.ownedHives[hiveIndex], null, 2));
