...
const { isValidObjectId, sanitizeString } = require('../utils/validation.js');
...
module.exports = async function handler(req, res) {
  ...
  if (basePath === '' && method === 'GET') {
    try {
      ...
      // Calculate unread count for current user and format response
      const conversationsWithUnread = conversations.map(conv => {
        try {
          ...
          console.error('Error mapping conversation:', conv._id, mapError);
      ...
      return res.json({
        success: true,
        conversations: conversationsWithUnread.filter(c => !c.error)
      });
    } catch (error) {
    ...
    return new Promise((resolve) => {
      upload.array('files', 5)(req, res, async (err) => {
        ...
        try {
          ...
          // Prepare files data - handle both Cloudinary and local storage
