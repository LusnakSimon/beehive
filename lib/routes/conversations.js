...
module.exports = async function handler(req, res) {
  ...
  if (basePath.match(/^\/[^/]+\/upload$/) && method === 'POST') {
    // Check if running on serverless (Vercel) - file uploads not supported without cloud storage
    const isServerless = process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME;
    if (isServerless) {
      return res.status(501).json({ 
        error: 'File upload is currently not available on this deployment. Please use cloud storage (Cloudinary/S3) integration.' 
      });
    }
