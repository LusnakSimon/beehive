...
module.exports = async function handler(req, res) {
  ...
  // Parse URL path - handle both /api/friends/... and direct calls
  let urlPath = req.url.split('?')[0]; // Remove query string
  if (urlPath.startsWith('/api/friends')) {
    urlPath = urlPath.replace('/api/friends', '');
  }
  ...
  if (basePath === '/request' && method === 'POST') {
    ...
    if (rateLimit(req, res, 'friendRequest')) return;
    ...
    try {
      ...
      try {
        await Notification.createFriendRequest(toUserId, auth.user.id, friendRequest._id);
      } catch (notifError) {
  ...
  if (basePath.match(/^\/requests\/[^/]+\/accept$/) && method === 'PATCH') {
    try {
      ...
      try {
        await Notification.createFriendRequestAccepted(
          friendRequest.from.toString(),
          friendRequest.to.toString()
        );
      } catch (notifError) {
        console.error('Error creating friend request accepted notification:', notifError);
        // Don't fail the request if notification creation fails
      }
