...
module.exports = async (req, res) => {
  try {
    ...
    if (method === 'POST') {
      const body = req.body || {};
      console.log('📥 POST /api/sensor received:', body);
      
      const { temperature, humidity, weight, battery, hiveId: bodyHiveId, metadata } = body;
      
      if (temperature === undefined || humidity === undefined || weight === undefined) {
        console.error('❌ Missing required fields');
        return res.status(400).json({ error: 'Temperature, humidity and weight are required' });
      }
      ...
      const reading = new Reading({
        ...
        weight,
        battery: battery || 100,
        hiveId: bodyHiveId || hiveId,
        metadata: metadata || {}
      });
      
      console.log('💾 Saving reading to MongoDB:', reading.toObject());
      await reading.save();
      console.log('✅ Reading saved successfully with ID:', reading._id);
