...
module.exports = async (req, res) => {
  try {
    ...
    if (method === 'POST') {
      ...
      // Must have either valid device API key or legacy key
      if (!deviceAuth.valid && !hasLegacyKey) {
        return res.status(401).json({ 
          error: 'Unauthorized',
          message: 'Valid device API key required' 
        });
      }
    ...
    // Require hiveId for GET requests
    if (!hiveId) {
