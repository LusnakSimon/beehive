...
const storage = multer.diskStorage({
  destination: async function (req, file, cb) {
    // Use /tmp on serverless environments (Vercel), otherwise use public/uploads
    const isServerless = process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME;
    const uploadDir = isServerless 
      ? path.join('/tmp', 'uploads', 'groups')
      : path.join(process.cwd(), 'public', 'uploads', 'groups');
  ...
  if (urlPath === '' && method === 'POST') {
    return new Promise((resolve) => {
      upload.fields([
        ...
        try {
          // Check if running on serverless and files were uploaded
          const isServerless = process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME;
          if (isServerless && (req.files?.coverImage || req.files?.icon)) {
            console.warn('File upload attempted on serverless environment - files stored in /tmp will not be accessible');
            return resolve(res.status(501).json({ 
              error: 'Image upload is currently not available on this deployment. Please use cloud storage (Cloudinary/S3) integration or create group without images for now.' 
            }));
          }
