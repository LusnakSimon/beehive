...
const { isValidObjectId, escapeRegex, sanitizeString } = require('../utils/validation.js');
...
module.exports = async function handler(req, res) {
        ...
        if (!conversation) {
          ...
          for (const memberId of memberIds) {
            if (memberId.toString() !== auth.user.id) {
              ...
              conversation.unreadCount.set(auth.user.id.toString(), 0);
            }
          }
  ...
  if (basePath.match(/^\/[^/]+\/read$/) && method === 'POST') {
    try {
      const groupId = pathParts[0];
      ...
      const isMember = group.members.some(m => m.userId.toString() === auth.user.id);
      if (!isMember) {
        return res.status(403).json({ error: 'Not a member of this group' });
      }

      // Find or create group conversation and reset unread count
      const Conversation = require('../models/Conversation');
      ...
      await GroupMessage.updateMany(
        {
          group: groupId,
          'readBy.userId': { $ne: auth.user.id }
        },
        {
          $addToSet: { readBy: { userId: auth.user.id, readAt: new Date() } }
        }
      );
      ...
      return res.status(500).json({ error: 'Failed to mark group as read' });
    }
  }
