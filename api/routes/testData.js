...
router.post('/generate', async (req, res) => {
  try {
    const { days = 7, pointsPerDay = 24, hiveIds = ['HIVE-001', 'HIVE-002', 'HIVE-003'] } = req.body
    
    // Clear existing test data for specified hives
    await Reading.deleteMany({ hiveId: { $in: hiveIds } })
    
    const allReadings = []
    const now = new Date()
    
    // Generate data for each hive
    for (const hiveId of hiveIds) {
      const readings = []
      
      // Add some variation between hives
      const hiveOffset = hiveIds.indexOf(hiveId) * 1.5
      ...
      for (let day = days - 1; day >= 0; day--) {
        for (let hour = 0; hour < pointsPerDay; hour++) {
          ...
          const baseTemp = 33 + hiveOffset // Slight variation between hives
          ...
          const baseHumidity = 55 - hiveOffset // Slight variation between hives
          ...
          const baseWeight = 45 + hiveOffset * 2
    ...
    res.json({
      success: true,
      message: `Generated ${allReadings.length} test readings for ${hiveIds.length} hives`,
      count: allReadings.length,
      hives: hiveIds,
      perHive: allReadings.length / hiveIds.length,
      dateRange: {
        from: allReadings[0].timestamp,
        to: allReadings[allReadings.length - 1].timestamp
      }
    })
  } catch (error) {
